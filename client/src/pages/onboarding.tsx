...
export default function OnboardingPage() {
  ...
  const handleStep8Submit = async (e: React.FormEvent) => {
    ...
    try {
      // Store the final complete school setup with bank account info
      const finalCompleteSchoolData = {
        name: schoolName,
        country,
        stateProvince,
        townDistrict,
  ...
  const addBankAccount = () => {
    if (currentBankData.bankName && currentBankData.branchName && currentBankData.accountNumber) {
      setBankAccounts(prev => [...prev, { ...currentBankData }]);
      setCurrentBankData({
        bankName: '',
        branchName: '',
        accountNumber: '',
...
Contact: ${Array.isArray(contactNumbers) ? contactNumbers.join(', ') : contactNumbers ? contactNumbers : '[Contact Numbers]'}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-teal-900 flex items-center justify-center px-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-4xl relative z-10">
          {/* Header */}
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Bank Account Info</h1>
            <p className="text-slate-400">Press the Button below to Add Bank Details</p>
          </div>
            ...
            <form onSubmit={handleStep8Submit} className="space-y-6">
              ...
              <div className="space-y-4">
                {bankAccounts.length === 0 && !showBankForm ? (
                  ...
                  <div className="border-2 border-dashed border-slate-500/40 rounded-xl bg-slate-800/20 backdrop-blur-sm p-12 text-center">
                    <button
                      type="button"
                      onClick={() => setShowBankForm(true)}
                      className="inline-flex items-center justify-center w-24 h-24 bg-slate-600/30 hover:bg-slate-500/40 rounded-xl transition-all duration-200 mb-4"
                    >
                      <svg className="w-12 h-12 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 4v16m8-8H4" />
                      </svg>
                    </button>
                    <p className="text-slate-400 text-sm">Click to add your first bank account</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {/* Existing Bank Accounts */}
                    {bankAccounts.map((account, index) => (
                      <div key={index} className="bg-slate-700/30 backdrop-blur-sm rounded-xl p-6 border border-slate-600/20">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4">
                            <div className="w-12 h-12 bg-slate-600/40 rounded-full flex items-center justify-center">
                            ...
                            <div>
                              ...
                              <div className="text-slate-400 text-sm space-y-1">
                                ...
                                <p>Branch: {account.branchName} | Sort Code: {account.sortCode}</p>
                              </div>
                            </div>
                          </div>
                          <button
                    ...
                    {!showBankForm && (
                      <button
                        type="button"
                        onClick={() => setShowBankForm(true)}
                        className="w-full border-2 border-dashed border-slate-500/40 rounded-xl bg-slate-800/20 backdrop-blur-sm p-6 text-center hover:bg-slate-700/20 transition-all duration-200"
                      >
                ...
                {/* Bank Form */}
                {showBankForm && (
                  <div className="bg-slate-700/20 backdrop-blur-sm rounded-xl p-6 border border-slate-600/20 space-y-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-white font-medium">Add Bank Account</h3>
                      <button
                        type="button"
                    ...
                    {/* Bank Selection */}
                    <div>
                      <label className="block text-slate-300 font-medium mb-2">Select your Bank</label>
                      <select
                        value={currentBankData.bankName}
                        onChange={(e) => setCurrentBankData(prev => ({ ...prev, bankName: e.target.value }))}
                    ...
                    {/* Branch Name */}
                    <div>
                      <label className="block text-slate-300 font-medium mb-2">Select your Branch</label>
                      <input
                        type="text"
                        value={currentBankData.branchName}
                        onChange={(e) => setCurrentBankData(prev => ({ ...prev, branchName: e.target.value }))}
                    ...
                    {/* Account Number */}
                    <div>
                      <label className="block text-slate-300 font-medium mb-2">Enter your bank account number</label>
                      <input
                        type="text"
                        value={currentBankData.accountNumber}
                        onChange={(e) => setCurrentBankData(prev => ({ ...prev, accountNumber: e.target.value }))}
                    ...
                    {/* Account Name */}
                    <div>
                      <label className="block text-slate-300 font-medium mb-2">Account Name</label>
                      <input
                        type="text"
                        value={currentBankData.accountName}
                        onChange={(e) => setCurrentBankData(prev => ({ ...prev, accountName: e.target.value }))}
                    ...
                    {/* Sort Code */}
                    <div>
                      <label className="block text-slate-300 font-medium mb-2">Sort Code</label>
                      <input
                        type="text"
                        value={currentBankData.sortCode}
                        onChange={(e) => setCurrentBankData(prev => ({ ...prev, sortCode: e.target.value }))}
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                      <button
                        type="button"
                        onClick={() => setShowBankForm(false)}
                        className="px-6 py-2 bg-slate-600/30 text-slate-300 rounded-xl hover:bg-slate-500/30 transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        disabled={!currentBankData.bankName || !currentBankData.branchName || !currentBankData.accountNumber}
                        className="px-6 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                      >
                        Add Account
                      </button>
                    </div>
