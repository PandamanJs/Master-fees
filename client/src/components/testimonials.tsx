  ...
  return (
    <section className="py-20 bg-slate-50/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4 text-center max-w-3xl mx-auto mb-16">
        ...
        {testimonials.map((testimonial, index) => (
          <div key={index} className="bg-white/90 backdrop-blur-sm p-6 rounded-xl shadow-sm border border-slate-200/50 hover:shadow-lg transition-all duration-300 grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
        ...
        <div className="border-t border-slate-200 pt-16">
          <h3 className="text-center text-lg font-semibold text-black mb-8">Trusted by leading schools</h3>
          <div className="scrolling-content bg-white py-4 rounded-lg shadow-sm border border-slate-200">
            {/* Duplicate the list to ensure seamless looping */}
            {[...trustedSchools, ...trustedSchools, ...trustedSchools].map((school, index) => (
              <span
                key={index}
                className="inline-block text-black text-lg font-medium px-8 whitespace-nowrap"
              >
                {school}
          ...
          <div className="scrolling-content bg-slate-50 py-3 rounded-lg shadow-sm border border-slate-200 mt-4" style={{ animationDirection: 'reverse', animationDuration: '35s' }}>
            {[...trustedSchools.slice().reverse(), ...trustedSchools.slice().reverse(), ...trustedSchools.slice().reverse()].map((school, index) => (
              <span
                key={index}
                className="inline-block text-black text-base font-medium px-6 whitespace-nowrap opacity-80"
              >
                ★ {school} School
